...
{Body}
            ...
            public const string JsonDeserialization =
@"protected override void DeserializeField(ref JSONDeserializer deserializer, string fieldName)
{
    switch(fieldName)
    {
{Cases}
        default:
            ...
            public const string JsonDeserialization_Primitive =
@"case nameof({PropertyName}):
    __{PropertyName} = deserializer.ReadValue<{ClassName}>(fieldName);
    return;";

            public const string JsonDeserialization_Object =
@"case nameof({PropertyName}):
    deserializer.ReadObject(__{PropertyName}, typeof({ClassName}));
    __{PropertyName}.parent = this;
    return;";

            public const string JsonDeserialization_List =
@"case nameof({PropertyName}):
    ModelList<{InnerClassName}>.Deserialize(ref deserializer, __{PropertyName}, fieldName);
    return;";
            
            public const string PrimitiveProperty = 
@"{PrivateTypeName} __{PropertyName};
...
static string SourceDirectory => {SourceDirectory};
            ...
            string sourceDirectory)
        {
            ...
            StringBuilder jsonDeserializationCases = new StringBuilder();
            ...
            foreach (var property in definition.Properties)
            {
                ...
                string jsonDeserializationTemplate = null;
                if (property.Type == ModelPropertyType.Object)
                {
                    ...
                    jsonDeserializationTemplate = Templates.JsonDeserialization_Object;
                ...
                else if (property.Type == ModelPropertyType.List)
                {
                    ...
                    callbackInvocationBody.AppendLine("__" + property.Name + ".InvokeModelUpdatedCallbacks(force, onExceptionCallback);");
                    jsonDeserializationTemplate = Templates.JsonDeserialization_List;
                    forceMarkDirtyBody.AppendLine("((IModelInternal)__" + property.Name + ").ForceMarkDirty();");
                }
                else
                {
                ...
                if (jsonDeserializationTemplate != null)
                {
                    jsonDeserializationCases.AppendLine(
                        jsonDeserializationTemplate
                            .Replace("{PropertyName}", property.Name)
                            .Replace("{ClassName}", typeName)
                            .Replace("{InnerClassName}", property.ClassName));
                }
            ...
            var jsonDeserialization = new StringBuilder();
            if (0 < jsonDeserializationCases.Length)
            {
                Indent(jsonDeserializationCases, 2);
                jsonDeserialization.Append(Templates.JsonDeserialization
                    .Replace("{Cases}", jsonDeserializationCases.ToString().TrimEnd()));
                Indent(jsonDeserialization, indentLevel);
            }
